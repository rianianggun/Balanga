    ...
    <div className="rounded-xl border border-border p-4 space-y-3 bg-white" data-testid={`score-card-${ind.id}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          {scores && <div className="text-[11px] text-muted-foreground mt-0.5 font-mono">Deretan skor: {scores.map((s, i) => `${KELAS[i]}=${s}`).join(" · ")}</div>}
        </div>
        {upload && <Button variant="ghost" size="sm" className="gap-1.5 shrink-0 text-emerald-700" data-testid={`penilai-view-file-${ind.id}`} onClick={() => onOpenFile(upload.file_id)}><FileText className="w-4 h-4" /> Berkas</Button>}
      </div>
      {ai && (
        <div className={`rounded-lg px-3 py-2 text-xs flex flex-wrap items-center justify-between gap-2 ${ai.kelas ? "bg-violet-50 border border-violet-200 text-violet-900" : "bg-slate-50 border border-border text-slate-600"}`} data-testid={`ai-hint-${ind.id}`}>
          <div className="flex items-start gap-2 min-w-0">
            ...
            <div>
              {ai.kelas ? <span className="font-semibold">Rekomendasi AI: kelas {ai.kelas.toUpperCase()} → skor {ai.score}</span> : <span className="font-semibold">AI: tidak ada rekomendasi</span>}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-12 gap-2 items-center">
        ...
        {scores ? (
          <Select value={row.kelas || ""} onValueChange={pickKelas}>
            <SelectTrigger className="sm:col-span-3" data-testid={`kelas-${ind.id}`}><SelectValue placeholder="Pilih kelas" /></SelectTrigger>
            <SelectContent>{KELAS.map((k, i) => <SelectItem key={k} value={k} data-testid={`kelas-${ind.id}-${k}`}>Kelas {k.toUpperCase()} — {scores[i]}</SelectItem>)}</SelectContent>
          </Select>
        ) : (
