...
## Re-setup from `rianianggun/Balanga` branch `Clone2` @ `9567035` (2026-09-25)
- `/app` was a bare template (not balanga1); synced via rsync, no git remote configured. Full log in `memory/setup_clone2.md`.
- All runtime pins kept (emergentintegrations 0.2.0 installed separately due to pip direct-URL conflict). Repo has no `yarn.lock` → plain `yarn install`, lockfile generated.
- `.env`: added `JWT_SECRET`, `FRONTEND_URL`, `ADMIN_EMAIL/PASSWORD`, `SEED_DEMO`, `EMERGENT_LLM_KEY`. Stripe not read by code.
- Smoke: startup clean (406 indikator, storage, demo seed), admin login JWT OK, login page renders. `backend_test.py` 52/54; `pytest backend/tests` 25 passed / 17 errors (hardcoded non-seeded admin). `GET /api/` is 404 (no root route).
- TODO: rotate `EMERGENT_LLM_KEY`; set git remote.
